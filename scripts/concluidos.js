...
function buscar() {
    firestore.collection("Tasks").where("status", "==", "concluido").onSnapshot((query) => {
        ...
        $.each(list, function (index, data) {
            dataSet.push([conta, data.id, data.status, data.tipo, data.urgencia, data.databaixa, data.uidfunc]);
            conta++;
        });
        console.log(list)
        ...
        $('#tb_dados').DataTable({
            ...
            columns: [
                ...
                { title: 'Baixa', className: 'baixa' },
                { title: 'Responsavel', className: 'responsavel' },
            ...
            if (ultimo !== atual) {
                ...
                firestore.collection("Tasks").doc(idAtual).get().then((doc) => {
                    if (doc.exists) {
                        ...
                        document.getElementById('textFuncionario').innerHTML = dado.uidfunc;
